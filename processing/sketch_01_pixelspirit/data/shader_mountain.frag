...
void main() {
	...
	vec3 color 		= vec3( 0.0 );
	// We are rotating the entire coordinate system 45 degrees, so that it rotates all the shapes, etc.
	st 				= rotate( st, radians(-45));
	// We have a float called offset that we are assinging the value of .12.
	float offset 	= .12;
	// We are defining the size of are two smaller shapes.
	vec2 size 		= vec2(1.0);
	// We are drawing two rectangles that are offset in the opposite directions along the x axis.
	color 			+= fill(rectSDF( st + offset, size ), .2);
	color 			+= fill(rectSDF( st - offset, size ), .2);
	// We are defining the size of the larger shape.
	float rect 		= rectSDF(st, size);
	color 			*= step(.33, rect);
	color 			+= fill(rect, .3);
